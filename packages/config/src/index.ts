...
import { EventManager } from "@terra-money/houston-events";
...
class HoustonConfig {
  ...
  public eventManagerOptions(config: HoustonConfig): any {
    let muteLogging;
  ...
  public with(obj: any): HoustonConfig {
    ...
    return assignIn(
      Object.create(HoustonConfig.prototype),
      current,
      normalized
    );
  }

  public merge(obj: any): HoustonConfig {
  ...
  public static default(): HoustonConfig {
    return new HoustonConfig();
  }
  ...
  public static detect(options: any = {}, filename?: string): HoustonConfig {
    let configFile;
    ...
    if (configPath) {
      ...
      configFile = HoustonConfig.search(options, filename);
    }
    ...
    return HoustonConfig.load(configFile, options);
  }

  public static load(file: string, options: any = {}): HoustonConfig {
    const workingDirectory = options.config
      ? process.cwd()
      : path.dirname(path.resolve(file));

    const config = new HoustonConfig(undefined, workingDirectory, undefined);
    ...
    const staticConfig = require(file);
  ...
  public static getTruffleDataDirectory(): string {
    const configStore = HoustonConfig.getUserConfig();

    return path.dirname(configStore.path);
  }
}

export default HoustonConfig;
