...
export const copyFiles = async (
  ...
  const sourcePath = path.join(__dirname, "../contractSource");
  ...
  await events.emit("add:smartContractCargo", {
...
export const changeName = (contractsDir: string, name: string) => {
  const newDir = `${contractsDir}/${name}`;
  // Change contract directory name
  const contractDir = `${contractsDir}/_`;
